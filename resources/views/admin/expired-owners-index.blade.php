        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                                                ...
                                                <form id="delete_{{ $owner->id }}" method="post"
                                                    ...
                                                    <td class="px-4 py-3">
                                                        <a data-id="{{ $owner->id }}" onclick="deletePost(this)"
                                                            class=" text-white bg-red-300 border-0 py-2 px-4 focus:outline-none hover:bg-red-500 rounded ">完全に削除</a>
                                                    </td>
                                                </form>
                                            </tr>
                                        @endforeach
                    ...
                    {{-- エロクエント
                        @foreach ($e_all as $e_onwer)
                        {{$e_onwer->name}}
                        {{$e_onwer->created_at->diffForHumans()}}
                        @endforeach
                        <br>
